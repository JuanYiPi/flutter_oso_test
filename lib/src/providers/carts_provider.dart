...
class CartsProvider {
  ...
  Future<List<Cart>> getAllPurchases() async {
    ...
    final url = Uri.https(authority, '${prefs.rutaAlmacen}api/api/users/${prefs.idUsuario}/carts', {
      'api_key'               : apiKey,
    });
  ...
  Future<List<CartDetail>> getCartDetailList({String cartId}) async {
    final url = Uri.https(authority, '${prefs.rutaAlmacen}api/api/carts/$cartId/details', {'api_key': apiKey});
  ...
  getNumbOfItemOfCart() async {
    ...
    final url = Uri.https(authority, '${prefs.rutaAlmacen}api/api/users/${prefs.idUsuario}/cartdetails', {
      'api_key': apiKey
    });
  ...
  Future<Cart> getActiveCart() async {
    final url = Uri.https(authority, '${prefs.rutaAlmacen}api/api/users/${prefs.idUsuario}/cartactive', {
      'api_key': apiKey
    });
  ...
  Future<bool> deleteFromShoppingCart(CartDetail item) async {
    final url = Uri.https(authority, '${prefs.rutaAlmacen}api/api/cartdetail/${item.id}', {'api_key': apiKey});
  ...
  Future<bool> updateShoppingCart({String directionId, String estado, String gastos, String mEntrega, String payReference}) async {
    final url = Uri.https(authority, '${prefs.rutaAlmacen}api/api/carts/${prefs.idActiveCart}');
